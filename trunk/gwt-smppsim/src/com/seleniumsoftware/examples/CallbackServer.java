...
public class CallbackServer {
	...
	public CallbackServer(int connections, int port, CallbackReceivable receiver)
			throws Exception {
		...
		try {
			...
			logger.severe("Exception creating CallbackServer server: "
					+ e.toString());
			e.printStackTrace();
			throw e;
		}
		for (int i = 0; i < connections; i++) {
			...
			callbackThread[threadIndex] = new Thread(
					callbackHandlers[threadIndex], "CH" + threadIndex);
