 ...
 * $Header: /var/cvsroot/SMPPSim2/src/java/com/seleniumsoftware/SMPPSim/TestProtocolHandler1.java,v 1.5 2007/12/06 16:21:36 martin Exp $
 ****************************************************************************/
...
public class TestProtocolHandler1 extends StandardProtocolHandler {
	...
	/**
	 * Custom variation of submitSM handler. This implementation will treat any
	 * destination MSISDN containing non-numeric characters as an invalid MSISDN
	 * and in such cases, return an error response of ESME_RINVDSTADR
	 */
	void getSubmitSMResponse(byte[] message, int len) throws Exception {
		...
		// Validate MSISDN
		try {
			logger.info("Validating destination_addr:"
					+ smppmsg.getDestination_addr());
			long n = Long.parseLong(smppmsg.getDestination_addr());
			// MSISDN is numeric, so treat it as "valid" for the purposes of
			// this test
			...
			logger
					.warning("'Invalid' MSISDN. This protocol handler treats non-numeric MSISDN as invalid for testing purposes");
			smsc.incSubmitSmERR();
			...
			LoggingUtilities.hexDump(
					"SUBMIT_SM_RESP following validation error:", resp_message,
					resp_message.length);
			if (Smsc.isDecodePdus())
				LoggingUtilities.logDecodedPdu(smppresp);
			connection.writeResponse(resp_message);
